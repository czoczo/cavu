...
curl https://raw.githubusercontent.com/czoczo/casavue/main/deployment/docker-compose/docker-compose.yaml > docker-compose.yaml
```
  </TabItem>
  <TabItem label="wget">
```console
mkdir CasaVue && cd CasaVue && \
wget https://raw.githubusercontent.com/czoczo/casavue/main/deployment/docker-compose/docker-compose.yaml
```
  </TabItem>
</Tabs>
...
```console
COLUMNS="$(tput cols)" LINES="$(tput lines)" docker compose run edit-main; reset
```
Editing `items.yaml` config file
```console
COLUMNS="$(tput cols)" LINES="$(tput lines)" docker compose run edit-items; reset
```
...
<Code code={importedDockerCompose.replaceAll('latest',importedAppVersion.trim()+'-alpine')} lang="yaml" title="docker-compimport type { transpileModule } from 'typescript';
ose.yaml" />