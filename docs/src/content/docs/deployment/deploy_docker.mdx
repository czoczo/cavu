...
Dynamic discovery of items works only with [Kubernetes](https://kubernetes.io). While running on plain Docker, items displayed on CasaVue may only be set using [static YAML configuration file](/configuration/file#static-items-definitions).
...
curl https://...raw/docker-compose.yaml
```
  </TabItem>
  <TabItem label="wget">
```console
mkdir CasaVue && cd CasaVue && \
wget https://...raw/docker-compose.yaml
```
  </TabItem>
</Tabs>
...
<Aside type="tip">
  [Edit items configuration file](/deployment/20_deploy_docker/#editing-configuration) before running the command in order to influence content of generated dashboard.
</Aside>
